...
const builds = [
  ...
  // UMD prod entry
  {
    ...
    entryPoints: [
      resolveToRoot('src/index.umd-su.js')
    ],
...
const runBuild = {
  ...
  async fast () { // does NOT builds types
    ...
    // import('./build.vetur.js').then(({ generate }) => generate({ api, compact: true }))
    // import('./build.web-types.js').then(({ generate }) => generate({ compact: true }))
  ...
  //   const quasarLangIndex = await import('./build.lang.js').then(({ generate }) => generate())
  //   import('./build.types.js').then(({ generate }) => generate({ quasarLangIndex }))
  // },
